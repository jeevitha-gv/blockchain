...
include 'php/common/config.php';
  ...
  if(isset($_POST['submit']))
  {
   $passkey=$_POST['passkey'];
    $tipNo=$_POST['tipNo'];
      $sql="SELECT passkey,tipNo FROM Idea where passkey='$passkey' and tipNo='$tipNo'";
     $result=mysqli_query($link,$sql);
             if($rows=mysqli_fetch_assoc($result))
             {    
               if($rows['passkey']==$passkey && $rows['tipNo']==$tipNo)
               {
                  echo "Sucessfully";
                header("location:/blockchain/idea/followup.php?tipno=".$rows['tipNo']);
               }
             }
             else
               {
...
<script type="text/javascript">
      var phone = document.getElementById('passkey'),
    cleanPhoneNumber;
